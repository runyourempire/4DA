// GAME Component: notif-medium — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: flow ──
    {
        ...
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.050000, 2.000000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.050000, 2.000000);
    ...
    // ── Layer 2: frame ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.350000, 0.350000, 0.400000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 1: flow ──
    {
        ...
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(2.000000), 0.050000, 2.000000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(2.000000), 0.050000, 2.000000);
    ...
    // ── Layer 2: frame ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.350000, 0.350000, 0.400000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'intensity', target: 'flow', field: 'brightness', weight: 0.6 },
      { source: 'hover', target: 'flow', field: 'brightness', weight: 0.1 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class NotifMedium extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-notif-medium: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-notif-medium', NotifMedium);
})();
