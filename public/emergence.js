// GAME Component: emergence — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let signal_count = u.p_signal_count;
    ...
    // ── Layer 1: void ──
    {
        ...
        { let warp_x = fbm2(p * 1.200000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.400000, 0.040000);
        let warp_y = fbm2(p * 1.200000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.400000, 0.040000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.040000; }
        var sdf_result = fbm2((p * 1.800000 + vec2<f32>(time * 0.1, time * 0.07)), i32(3.000000), 0.400000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.005000, 0.003000, 0.010000), vec3<f32>(0.010000, 0.008000, 0.030000), vec3<f32>(0.030000, 0.020000, 0.060000), vec3<f32>(0.000000, 0.010000, 0.040000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(2.000000), (0.080000 + (metabolism * 0.030000)), 2.000000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(2.000000), (0.080000 + (metabolism * 0.030000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.080000 + (metabolism * 0.030000)); }
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.040000, 0.030000, 0.100000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.060000, 2.000000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.060000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.060000; }
        var sdf_result = voronoi2(p * 4.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = (0.300000 + (awareness * 0.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.080000, 0.060000, 0.020000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.100000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.100000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.100000; }
        var sdf_result = voronoi2(p * 8.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = (signal_count * 1.500000) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.120000, 0.090000, 0.020000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: cursor ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.060000, 0.050000, 0.150000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
        color_result = mix(color_result, prev_color, 0.700000);
...
uniform float u_p_signal_count;
...
void main(){
    ...
    float signal_count = u_p_signal_count;
    ...
    // ── Layer 1: void ──
    {
        ...
        { float warp_x = fbm2(p * 1.200000 + vec2(0.0, 1.3), int(2.000000), 0.400000, 0.040000);
        float warp_y = fbm2(p * 1.200000 + vec2(1.7, 0.0), int(2.000000), 0.400000, 0.040000);
        p = p + vec2(warp_x, warp_y) * 0.040000; }
        float sdf_result = fbm2((p * 1.800000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.400000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.005000, 0.003000, 0.010000), vec3(0.010000, 0.008000, 0.030000), vec3(0.030000, 0.020000, 0.060000), vec3(0.000000, 0.010000, 0.040000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(2.000000), (0.080000 + (metabolism * 0.030000)), 2.000000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(2.000000), (0.080000 + (metabolism * 0.030000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.080000 + (metabolism * 0.030000)); }
        ...
        color_result = vec4(color_result.rgb * vec3(0.040000, 0.030000, 0.100000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(2.000000), 0.060000, 2.000000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(2.000000), 0.060000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.060000; }
        float sdf_result = voronoi2(p * 4.000000 + vec2(time * 0.05, time * 0.03));
        float glow_pulse = (0.300000 + (awareness * 0.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.080000, 0.060000, 0.020000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.700000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(2.000000), 0.100000, 2.000000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(2.000000), 0.100000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.100000; }
        float sdf_result = voronoi2(p * 8.000000 + vec2(time * 0.05, time * 0.03));
        float glow_pulse = (signal_count * 1.500000) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.120000, 0.090000, 0.020000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.500000);
    ...
    // ── Layer 5: cursor ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.060000, 0.050000, 0.150000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.700000);
...
`;
const UNIFORMS = [{name:'metabolism',default:0.5},{name:'signal_count',default:0},{name:'awareness',default:0.3}];
...
`;
const PASS_WGSL_1 = `// Post-processing pass: depth
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'metabolism', target: 'streams', field: 'intensity', weight: 0.2 },
      { source: 'signal_count', target: 'sparks', field: 'brightness', weight: 0.4 },
      { source: 'awareness', target: 'cells', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'awareness', from: 0.15, to: 0.4, duration: 12, easing: 'ease_in_out' },
    ];
  }
...
class Emergence extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-emergence: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get metabolism() { return this._renderer?.userParams['metabolism'] ?? this._pendingParams['metabolism'] ?? 0.5; }
  set metabolism(v) { this.setParam('metabolism', v); }
  get signal_count() { return this._renderer?.userParams['signal_count'] ?? this._pendingParams['signal_count'] ?? 0; }
  set signal_count(v) { this.setParam('signal_count', v); }
...
customElements.define('game-emergence', Emergence);
})();
