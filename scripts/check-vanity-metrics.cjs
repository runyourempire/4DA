...
/**
 * Core matcher (exported for tests). True if the already-collapsed window text renders
 * one of the banned vanity-counter labels AS A COUNTER (adjacent to a number/{{count}}).
 * This is the pure decision the gate makes; `scanText`/`main` add the file plumbing,
 * the escape-hatch handling, and the exit codes.
 */
function isVanityCounter(windowText) {
  return PATTERNS.some((re) => re.test(windowText));
}
...
function scanText(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!isVanityCounter(windowText)) continue;
    const scope = lines.slice(Math.max(0, i - 1), i + 2).join('\n');
    if (/vanity-ok:/.test(scope)) {
      i += 1;
      continue;
    }
    out.push({ line: i + 1, snippet: lines[i].trim().slice(0, 100) });
    i += 1;
  }
  return out;
}
...
module.exports = { PATTERNS, isVanityCounter, scanText };
