...
const ADVANCED_TERMS: &[&str] = &[
    ...
    "merkle", "b-tree", "lsm", "bloom filter", "skip list",
    "futex", "spinlock", "rwlock", "atomic", "memory ordering",
    "cache coherence", "false sharing", "simd", "vectorization",
    "branch prediction", "prefetch", "memory-mapped", "io_uring",
    "epoll", "kqueue", "completion port",
    "dependent type", "higher-kinded", "existential type", "phantom type",
    "variance", "covariance", "contravariance", "type erasure",
    "monomorphize", "trait object", "dyn dispatch", "static dispatch",
    "lifetime elision", "borrow checker", "affine type", "linear type",
    "crdt", "vector clock", "lamport", "causal consistency",
    "eventual consistency", "strong consistency", "two-phase commit",
    "three-phase commit", "gossip protocol", "anti-entropy",
    "timing attack", "side channel", "constant-time", "memory safety",
    "use-after-free", "buffer overflow", "stack canary", "aslr",
    "sandboxing", "capability-based", "zero-knowledge",
    "eigenvalue", "gradient descent", "backpropagation", "transformer",
    "attention mechanism", "embedding space", "dimensionality reduction",
    "kernel trick", "regularization", "overfitting",
    "quic", "http/3", "tls 1.3", "mtls", "service mesh",
    "grpc", "protobuf", "flatbuffers", "cap theorem",
    "write-ahead log", "wal", "mvcc", "snapshot isolation",
    "serializable", "vacuum", "compaction", "tombstone",
];

const BEGINNER_TITLE_TERMS: &[&str] = &[
    "getting started", "for beginners", "beginner's guide", "beginners guide",
    "your first", "hello world", "from scratch", "made easy", "simplified",
    "for dummies", "crash course", "in 5 minutes", "in 10 minutes",
    "in 15 minutes", "quick start", "quickstart", "learn to", "learn how to",
    " 101", "basics of", "fundamentals of", "what is ", "intro to ",
    "introduction to ", "complete beginner", "absolute beginner",
    "step by step", "step-by-step", "full project breakdown",
];

const ABSTRACT_FRAMING: &[&str] = &[
    "principles of", "patterns in", "architecture of", "design of",
    "internals of", "under the hood", "deep dive", "in depth", "in-depth",
    "trade-offs", "tradeoffs", "trade offs", "beyond the basics",
    "advanced ", "production-grade", "production grade", "at scale",
    "high-performance", "high performance",
];

const CODE_SOPHISTICATION_MARKERS: &[&str] = &[
    "result<", "-> result", "anyhow::", "thiserror", "unwrap_or",
    "unwrap_or_else", "map_err", "async fn", "async move", ".await",
    "impl ", "trait ", "where ", "<t>", "<t,", "generic", "lifetime",
    "pin<", "arc<", "mutex<", "unsafe ", "try {", "try!(", "catch (",
    "promise<", "async/await", "observable",
];

const DEPTH_MARKERS: &[&str] = &[
    "trade-off", "tradeoff", "limitation", "edge case", "edge-case",
    "anti-pattern", "antipattern", "gotcha", "caveat", "pitfall",
    "when not to", "when to avoid", "downside", "drawback",
    "benchmark", "profiling", "flame graph", "perf ", "regression",
    "migration path",
];

const REFERENCE_MARKERS: &[&str] = &[
    "rfc ", "arxiv", "doi.org", "ieee", "acm.org", "spec ",
    "specification", "official doc", "documentation", "changelog",
    "release note", "github.com/",
];

const WALKTHROUGH_MARKERS: &[&str] = &[
    "step 1", "step 2", "step 3", "step one", "step two",
    "first, install", "first install", "npm install", "pip install",
    "let's create", "let's build", "let's start", "we'll create",
    "we will create", "open your terminal", "open your editor",
    "create a new file", "create a new project",
];
