...
mod tests {
    ...
    /// correct order.
    ///
    /// Strategy: create a "query" vector Q, then insert 100 items whose
    /// embeddings have varying cosine distances from Q. The KNN search
    /// should return items sorted by ascending distance.
    ///
    /// NOTE: Requires sqlite-vec extension loaded at runtime. If the vec0
    /// virtual tables fail to create (e.g., in minimal CI environments),
    ...
    fn stress_knn_search_accuracy() {
        ...
        // Item i has embedding = normalize(query_vec + noise_i), where noise_i grows with i.
        // So item 0 is closest to query_vec, item 99 is farthest.
        let mut expected_order: Vec<(i64, f32)> = Vec::new();

        for i in 0..100 {
            let mut emb = query_vec.clone();
            // Add increasing noise to dimensions 2..dim
            let noise_scale = (i as f32 + 1.0) * 0.05;
            for d in 2..dim {
                emb[d] = noise_scale * ((d as f32 * 0.1 + i as f32 * 0.37).sin());
            }
            // Reduce alignment with query on dimension 0 as i grows
            ...
            // Compute L2 distance to query for expected ordering
            let dist: f32 = emb
                .iter()
                .zip(query_vec.iter())
                .map(|(a, b)| (a - b) * (a - b))
