// GAME Component: radar-field — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let stack_glow = u.p_stack_glow;
    let moving_in = u.p_moving_in;
    let lang_energy = u.p_lang_energy;
    let fw_energy = u.p_fw_energy;
    let tool_energy = u.p_tool_energy;
    let plat_energy = u.p_plat_energy;
    let moving_out = u.p_moving_out;
    ...
    // ── Layer 0: atmosphere ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.080000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.080000, 2.000000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.030000, 0.030000, 0.040000), vec3<f32>(0.080000, 0.080000, 0.100000), vec3<f32>(0.400000, 0.350000, 0.500000), vec3<f32>(0.000000, 0.050000, 0.150000));
    ...
    // ── Layer 1: rings ──
    {
        ...
        var sdf_result = fbm2((p * 6.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(2.000000), 0.300000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.020000, 0.020000, 0.030000), vec3<f32>(0.060000, 0.050000, 0.080000), vec3<f32>(0.300000, 0.250000, 0.400000), vec3<f32>(0.000000, 0.100000, 0.200000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 2: core ──
    {
        ...
        let glow_pulse = stack_glow * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.850000, 0.750000, 0.350000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.150000, 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.150000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.150000; }
        var sdf_result = noise2(p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(moving_in, 0.020000, 0.000000), vec3<f32>(0.200000, 0.150000, 0.050000), vec3<f32>(0.800000, 0.600000, 0.200000), vec3<f32>(0.000000, 0.100000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
...
uniform float u_p_stack_glow;
uniform float u_p_moving_in;
uniform float u_p_lang_energy;
uniform float u_p_fw_energy;
uniform float u_p_tool_energy;
uniform float u_p_plat_energy;
uniform float u_p_moving_out;
...
void main(){
    ...
    float stack_glow = u_p_stack_glow;
    float moving_in = u_p_moving_in;
    float lang_energy = u_p_lang_energy;
    float fw_energy = u_p_fw_energy;
    float tool_energy = u_p_tool_energy;
    float plat_energy = u_p_plat_energy;
    float moving_out = u_p_moving_out;
    ...
    // ── Layer 0: atmosphere ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(3.000000), 0.080000, 2.000000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(3.000000), 0.080000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.080000; }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.030000, 0.030000, 0.040000), vec3(0.080000, 0.080000, 0.100000), vec3(0.400000, 0.350000, 0.500000), vec3(0.000000, 0.050000, 0.150000));
    ...
    // ── Layer 1: rings ──
    {
        ...
        float sdf_result = fbm2((p * 6.000000 + vec2(time * 0.1, time * 0.07)), int(2.000000), 0.300000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.020000, 0.020000, 0.030000), vec3(0.060000, 0.050000, 0.080000), vec3(0.300000, 0.250000, 0.400000), vec3(0.000000, 0.100000, 0.200000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
    ...
    // ── Layer 2: core ──
    {
        ...
        float glow_pulse = stack_glow * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.850000, 0.750000, 0.350000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(2.000000), 0.150000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(2.000000), 0.150000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.150000; }
        float sdf_result = noise2(p * 3.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(moving_in, 0.020000, 0.000000), vec3(0.200000, 0.150000, 0.050000), vec3(0.800000, 0.600000, 0.200000), vec3(0.000000, 0.100000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
...
`;
const UNIFORMS = [{name:'stack_glow',default:0},{name:'moving_in',default:0},{name:'lang_energy',default:0.5},{name:'fw_energy',default:0.5},{name:'tool_energy',default:0.5},{name:'plat_energy',default:0.5},{name:'moving_out',default:0.1}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'moving_in', target: 'core', field: 'brightness', weight: 0.5 },
      { source: 'stack_glow', target: 'atmosphere', field: 'brightness', weight: 0.3 },
      { source: 'lang_energy', target: 'rings', field: 'brightness', weight: 0.2 },
      { source: 'fw_energy', target: 'energy', field: 'brightness', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class RadarField extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-radar-field: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get stack_glow() { return this._renderer?.userParams['stack_glow'] ?? 0; }
  set stack_glow(v) { this.setParam('stack_glow', v); }
  get moving_in() { return this._renderer?.userParams['moving_in'] ?? 0; }
  set moving_in(v) { this.setParam('moving_in', v); }
  get lang_energy() { return this._renderer?.userParams['lang_energy'] ?? 0; }
  set lang_energy(v) { this.setParam('lang_energy', v); }
  get fw_energy() { return this._renderer?.userParams['fw_energy'] ?? 0; }
  set fw_energy(v) { this.setParam('fw_energy', v); }
  get tool_energy() { return this._renderer?.userParams['tool_energy'] ?? 0; }
  set tool_energy(v) { this.setParam('tool_energy', v); }
  get plat_energy() { return this._renderer?.userParams['plat_energy'] ?? 0; }
  set plat_energy(v) { this.setParam('plat_energy', v); }
  get moving_out() { return this._renderer?.userParams['moving_out'] ?? 0; }
  set moving_out(v) { this.setParam('moving_out', v); }
...
customElements.define('game-radar-field', RadarField);
})();
