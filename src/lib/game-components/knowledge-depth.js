// GAME Component: knowledge-depth — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let coverage = u.p_coverage;
    let depth_val = u.p_depth_val;
    ...
    // ── Layer 0: terrain ──
    {
        ...
        var sdf_result = fbm2((p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.550000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.050000, 0.050000, 0.080000), vec3<f32>(coverage, 0.300000, 0.200000), vec3<f32>(1.000000, 0.700000, 0.500000), vec3<f32>(0.000000, 0.150000, 0.300000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 6.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.120000, 2.000000);
        let warp_y = fbm2(p * 6.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.120000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.120000; }
        var sdf_result = noise2(p * 3.500000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.020000, 0.020000, 0.040000), vec3<f32>(0.150000, 0.120000, 0.100000), vec3<f32>(0.800000, 0.600000, 0.400000), vec3<f32>(0.250000, 0.100000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = sdf_circle(p, 0.600000);
        let glow_pulse = depth_val * (0.9 + 0.1 * sin(time * 2.0));
...
uniform float u_p_coverage;
uniform float u_p_depth_val;
...
void main(){
    ...
    float coverage = u_p_coverage;
    float depth_val = u_p_depth_val;
    ...
    // ── Layer 0: terrain ──
    {
        ...
        float sdf_result = fbm2((p * 5.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.550000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.050000, 0.050000, 0.080000), vec3(coverage, 0.300000, 0.200000), vec3(1.000000, 0.700000, 0.500000), vec3(0.000000, 0.150000, 0.300000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 6.000000 + vec2(0.0, 1.3), int(3.000000), 0.120000, 2.000000);
        float warp_y = fbm2(p * 6.000000 + vec2(1.7, 0.0), int(3.000000), 0.120000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.120000; }
        float sdf_result = noise2(p * 3.500000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.020000, 0.020000, 0.040000), vec3(0.150000, 0.120000, 0.100000), vec3(0.800000, 0.600000, 0.400000), vec3(0.250000, 0.100000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = sdf_circle(p, 0.600000);
        float glow_pulse = depth_val * (0.9 + 0.1 * sin(time * 2.0));
...
`;
const UNIFORMS = [{name:'coverage',default:0},{name:'depth_val',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'coverage', target: 'terrain', field: 'brightness', weight: 0.5 },
      { source: 'depth_val', target: 'glow_center', field: 'brightness', weight: 0.6 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class KnowledgeDepth extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-knowledge-depth: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get coverage() { return this._renderer?.userParams['coverage'] ?? 0; }
  set coverage(v) { this.setParam('coverage', v); }
  get depth_val() { return this._renderer?.userParams['depth_val'] ?? 0; }
  set depth_val(v) { this.setParam('depth_val', v); }
...
customElements.define('game-knowledge-depth', KnowledgeDepth);
})();
