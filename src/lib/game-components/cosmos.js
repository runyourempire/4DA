// GAME Component: cosmos — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, 0.300000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, 0.300000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_a = sdf_circle(p, 0.100000);
        let sdf_b = abs(length(p) - 0.180000) - 0.030000;
        var sdf_result = smin(sdf_a, sdf_b, 0.060000);
        let glow_pulse = 3.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.850000, 0.500000), 1.0);
    ...
    // ── Layer 3: accretion ──
    {
        ...
        var sdf_result = abs(length(p) - 0.300000) - 0.020000;
        let glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.900000, 0.400000, 0.200000), 1.0);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(5.000000), 0.500000, 0.300000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(5.000000), 0.500000, 0.300000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_a = sdf_circle(p, 0.100000);
        float sdf_b = abs(length(p) - 0.180000) - 0.030000;
        float sdf_result = smin(sdf_a, sdf_b, 0.060000);
        float glow_pulse = 3.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.850000, 0.500000), 1.0);
    ...
    // ── Layer 3: accretion ──
    {
        ...
        float sdf_result = abs(length(p) - 0.300000) - 0.020000;
        float glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.900000, 0.400000, 0.200000), 1.0);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
...
`;
const UNIFORMS = [{name:'expansion',default:0},{name:'energy',default:0},{name:'rotation',default:0.3}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'expansion', target: 'star', field: 'scale', weight: 0.4 },
      { source: 'expansion', target: 'accretion', field: 'scale', weight: 0.6 },
      { source: 'expansion', target: 'halo', field: 'scale', weight: 0.3 },
      { source: 'energy', target: 'star', field: 'brightness', weight: 0.5 },
      { source: 'energy', target: 'particles', field: 'brightness', weight: 0.7 },
      { source: 'rotation', target: 'accretion', field: 'brightness', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'expansion', from: 0, to: 1, duration: 12, easing: 'ease_in_out' },
      { target: 'energy', from: 0, to: 0.8, duration: 6, easing: 'ease_out' },
      { target: 'rotation', from: 0.3, to: 0.8, duration: 8, easing: 'ease_in_out' },
    ];
  }
...
class Cosmos extends HTMLElement {
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-cosmos: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get expansion() { return this._renderer?.userParams['expansion'] ?? 0; }
  ...
  get rotation() { return this._renderer?.userParams['rotation'] ?? 0; }
  set rotation(v) { this.setParam('rotation', v); }
...
customElements.define('game-cosmos', Cosmos);
})();
