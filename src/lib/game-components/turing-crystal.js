// GAME Component: turing-crystal — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.020000, 0.020000, 0.060000), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.020000, 0.020000, 0.060000), 1.0);
...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let abb = a * b * b;
    let new_a = a + (1 * lap.x - abb + 0.025 * (1.0 - a));
    let new_b = b + (0.5 * lap.y + abb - (0.025 + 0.06) * b);
...
class GameReactionField {
  ...
  async init() {
    ...
    // Seed: random field, density 0.03
    for (let i = 0; i < cellCount; i++) {
      if (Math.random() < 0.03) init[i * 2 + 1] = 1.0;
    }
...
class TuringCrystal extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-turing-crystal: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-turing-crystal', TuringCrystal);
})();
