...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 2: track ──
    {
        ...
        var sdf_result = sdf_box(p, 0.920000, 0.820000);
        sdf_result = sdf_result - 0.040000;
        sdf_result = abs(sdf_result) - 0.001000;
        let glow_pulse = 0.080000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.120000, 0.030000, 0.020000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = sdf_box(p, 0.940000, 0.840000);
        sdf_result = sdf_result - 0.050000;
        sdf_result = abs(sdf_result) - 0.015000;
        let glow_pulse = 0.300000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.250000, 0.040000, 0.030000), color_result.a);
...
void main(){
    ...
    // ── Layer 1: sweep ──
    {
        ...
        float sdf_result = sdf_box(p, 0.920000, 0.820000);
        sdf_result -= 0.040000;
        sdf_result = abs(sdf_result) - 0.004000;
        float arc_theta = atan(p.x, p.y) + 3.14159265359;
        sdf_result = (arc_theta < 0.400000 ? sdf_result : 999.0);
    ...
    // ── Layer 2: track ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.120000, 0.030000, 0.020000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = sdf_box(p, 0.940000, 0.840000);
        sdf_result -= 0.050000;
        sdf_result = abs(sdf_result) - 0.015000;
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.250000, 0.040000, 0.030000), color_result.a);
