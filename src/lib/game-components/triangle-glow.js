// GAME Component: triangle-glow — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var sdf_result = sdf_triangle(p, 0.300000);
    let glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
    let glow_result = apply_glow(sdf_result, glow_pulse);
    var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.400000, 0.900000, 0.600000), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    float sdf_result = sdf_triangle(p, 0.300000);
    float glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
    float glow_result = apply_glow(sdf_result, glow_pulse);

    vec4 color_result = vec4(vec3(glow_result), 1.0);
    color_result = vec4(color_result.rgb * vec3(0.400000, 0.900000, 0.600000), 1.0);
...
class TriangleGlow extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-triangle-glow: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-triangle-glow', TriangleGlow);
})();
