// game-boot-ring.js — Generated by GAME compiler v0.2.0
// Zero-dependency WebGPU component with WebGL2 fallback.
//   <script type="module" src="./game-boot-ring.js"></script>
//   <game-boot-ring></game-boot-ring>
...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    // ── Layer 0: track ──
    {
        ...
        // stage 0: ring(...)
        var sdf_result = abs(length(p) - 0.3) - 0.015;
    ...
    // ── Layer 1: fill ──
    {
        var p = vec2f(uv.x * aspect, uv.y);
        // stage 0: ring(...)
        var sdf_result = abs(length(p) - 0.3) - 0.025;
...
void fs_main(input: VertexOutput){
    ...
    // ── Layer 0: track ──
    {
        ...
        // stage 0: ring(...)
        float sdf_result = abs(length(p) - 0.3) - 0.015;

        // stage 1: glow(...)
        float glow_result = apply_glow(sdf_result, 1.0);
    ...
    // ── Layer 1: fill ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        // stage 0: ring(...)
        float sdf_result = abs(length(p) - 0.3) - 0.025;
...
class GameBootRing extends HTMLElement {
  ...
  _frame() {
    ...
    this._paramValues[1] = (this._arcBaseOverrides?.[1] ?? 2) + (2.0 + (Math.sin((time * 2.0)) * 0.5));
...
customElements.define('game-boot-ring', GameBootRing);
export { GameBootRing };
export default GameBootRing;
