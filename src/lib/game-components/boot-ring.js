...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: texture ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.050000, 0.030000, 0.010000), vec3<f32>(0.300000, 0.180000, 0.060000), vec3<f32>(1.000000, 0.700000, 0.400000), vec3<f32>(0.000000, 0.100000, 0.200000));
    ...
    // ── Layer 2: track ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.250000, 0.250000, 0.250000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    // ── Layer 4: edge ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.850000, 0.400000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 1: texture ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.050000, 0.030000, 0.010000), vec3(0.300000, 0.180000, 0.060000), vec3(1.000000, 0.700000, 0.400000), vec3(0.000000, 0.100000, 0.200000));
    ...
    // ── Layer 2: track ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.250000, 0.250000, 0.250000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    // ── Layer 4: edge ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.850000, 0.400000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
