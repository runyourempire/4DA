...
/**
 * `<game-notif-digest>` Web Component
 *
 * A self-contained WebGPU/WebGL2 shader component.
 *
 * @example
 * ```html
 * <game-notif-digest intensity="0" hover="0"></game-notif-digest>
 * ```
 *
 * @example
 * ```typescript
 * const el = document.querySelector('game-notif-digest')!;
   * el.intensity = 0;
   * el.hover = 0;
 * ```
 */
interface GameNotifDigestElement extends HTMLElement {
...
declare global {
  interface HTMLElementTagNameMap {
    'game-notif-digest': GameNotifDigestElement;
  }
}
