// GAME Component: resonance-demo — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: core ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.900000, 0.300000, 0.100000), 1.0);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = fbm2((p * 4.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(3.000000), 0.600000, 2.000000);
        let glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.800000, 0.900000), 1.0);
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb + lc, 1.0);
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.400000) - 0.015000;
        let glow_pulse = 1.200000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.850000, 0.250000), 1.0);
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb + lc, 1.0);
    }
...
void main(){
    ...
    // ── Layer 1: core ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.900000, 0.300000, 0.100000), 1.0);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = fbm2((p * 4.000000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.600000, 2.000000);
        float glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.800000, 0.900000), 1.0);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.400000) - 0.015000;
        float glow_pulse = 1.200000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.850000, 0.250000), 1.0);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    }
...
`;
const UNIFORMS = [{name:'bass',default:0},{name:'energy',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'bass', target: 'core', field: 'scale', weight: 0.3 },
      { source: 'energy', target: 'noise_field', field: 'intensity', weight: 0.5 },
      { source: 'bass', target: 'outer_ring', field: 'opacity', weight: 0.7 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class ResonanceDemo extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-resonance-demo: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get bass() { return this._renderer?.userParams['bass'] ?? 0; }
  set bass(v) { this.setParam('bass', v); }
...
customElements.define('game-resonance-demo', ResonanceDemo);
})();
