// GAME Component: temporal-demo — auto-generated, do not edit.
(function(){
...
`;
const UNIFORMS = [{name:'bass',default:0.5},{name:'beat',default:0}];
...
const _smooth_bass_0 = new GameEMAFilter(0.05);
const _delay_bass_1 = new GameRingBuffer(0.2);
const _trigger_beat = new GameEdgeDetector(0.3);

_val_bass = _smooth_bass_0.update(_val_bass);
_delay_bass_1.push(_val_bass); _val_bass = _delay_bass_1.delayed();
_val_bass = Math.min(Math.max(_val_bass, 0), 1);
_val_beat = _trigger_beat.update(_val_beat);


class TemporalDemo extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-temporal-demo: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get beat() { return this._renderer?.userParams['beat'] ?? 0; }
  set beat(v) { this.setParam('beat', v); }
...
customElements.define('game-temporal-demo', TemporalDemo);
})();
