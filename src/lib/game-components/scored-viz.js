// GAME Component: scored-viz — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var sdf_result = abs(length(p) - 0.400000) - 0.020000;
    let glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
    let glow_result = apply_glow(sdf_result, glow_pulse);
    var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.600000, 1.000000), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    float sdf_result = abs(length(p) - 0.400000) - 0.020000;
    float glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
    float glow_result = apply_glow(sdf_result, glow_pulse);

    vec4 color_result = vec4(vec3(glow_result), 1.0);
    color_result = vec4(color_result.rgb * vec3(0.800000, 0.600000, 1.000000), 1.0);
...
class GameScorePlayer {
  constructor() {
    this._timeline = [
      {target:'ring_viz.scale',from:0.5,to:2,start:0,dur:8,easing:'ease_in'},
      {target:'ring_viz.scale',from:2,to:0.5,start:8,dur:4,easing:'linear'},
      {target:'ring_viz.scale',from:0.5,to:2,start:12,dur:8,easing:'ease_in'},
      {target:'ring_viz.scale',from:2,to:0.5,start:20,dur:4,easing:'linear'},
    ];
    this._totalDur = 24;
    this._startTime = null;
  }

  start(time) { this._startTime = time; }
  ...
  evaluate(time) {
    ...
    for (const e of this._timeline) {
      if (t >= e.start && t < e.start + e.dur) {
        const p = (t - e.start) / e.dur;
        const ep = this._ease(p, e.easing);
        result[e.target] = e.from + (e.to - e.from) * ep;
      } else if (t >= e.start + e.dur) {
        result[e.target] = e.to;
      }
    }
    return result;
  }
...
class ScoredViz extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-scored-viz: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-scored-viz', ScoredViz);
})();
