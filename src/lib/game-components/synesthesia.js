// GAME Component: synesthesia — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.800000, 1.000000), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.200000, 0.800000, 1.000000), 1.0);
...
class GameVoiceSynth {
  constructor(ctx) {
    this._ctx = ctx;
    this._nodes = {};
    this._nodes['tone'] = ctx.createOscillator();
    this._nodes['tone'].type = 'sine';
    this._nodes['tone'].frequency.value = 440;
    this._nodes['filter'] = ctx.createBiquadFilter();
    this._nodes['filter'].type = 'lowpass';
    this._nodes['filter'].frequency.value = 2000;
    this._nodes['filter'].Q.value = 2;
    this._nodes['output'] = ctx.createGain();
    this._nodes['output'].gain.value = 0.3;
  }

  connect(destination) {
    const names = Object.keys(this._nodes);
    for (let i = 0; i < names.length - 1; i++) {
      this._nodes[names[i]].connect(this._nodes[names[i + 1]]);
    }
  ...
  setParam(nodeName, paramName, value) {
    const n = this._nodes[nodeName];
    if (!n) return;
    if (n[paramName] && n[paramName].value !== undefined) n[paramName].value = value;
    else if (n.frequency && paramName === 'freq') n.frequency.value = value;
    else if (n.gain && paramName === 'level') n.gain.value = value;
  }
...
class Synesthesia extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-synesthesia: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-synesthesia', Synesthesia);
})();
