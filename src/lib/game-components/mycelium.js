// GAME Component: mycelium — auto-generated, do not edit.
(function(){
...
fn cs_agent(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let sense_l = sample_trail(agent.pos.x + cos(agent.angle + 0.5235987755982988) * 7, agent.pos.y + sin(agent.angle + 0.5235987755982988) * 7);
    let sense_f = sample_trail(agent.pos.x + cos(agent.angle) * 7, agent.pos.y + sin(agent.angle) * 7);
    let sense_r = sample_trail(agent.pos.x + cos(agent.angle - 0.5235987755982988) * 7, agent.pos.y + sin(agent.angle - 0.5235987755982988) * 7);
...
fn cs_diffuse(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    trail_out[gid.y * params.width + gid.x] = (sum / 9.0) * 0.97;
}
`;
...
class GameSwarmSim {
  constructor(device, agentCode, trailCode) { this._count = 150000; this._w = 512; this._h = 512; this._device = device; this._agentCode = agentCode; this._trailCode = trailCode; }
  ...
  dispatch(dt) {
    ...
    f[0] = 0.5235987755982988; f[1] = 7; f[2] = 0.5235987755982988; f[3] = 0.8; f[4] = 6;
  ...
  get agentCount() { return 150000; }
}


class Mycelium extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-mycelium: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-mycelium', Mycelium);
})();
