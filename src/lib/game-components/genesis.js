// GAME Component: genesis — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.250000, 2.000000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.250000, 2.000000);
        ...
        var color_result = vec4<f32>(cosine_palette(sdf_result, vec3<f32>(0.020000, 0.010000, 0.050000), vec3<f32>(0.100000, 0.050000, 0.150000), vec3<f32>(1.000000, 0.800000, 0.600000), vec3<f32>(0.000000, 0.200000, 0.400000)), 1.0);
    ...
    // ── Layer 2: seed ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.600000, 0.200000), 1.0);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.250000) - 0.020000;
        let glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.400000, 0.900000), 1.0);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 5.000000 + time * 0.300000), cos(p.x * 5.000000 + time * 0.300000)) * 0.200000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.420000) - 0.005000;
        let glow_pulse = 1.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.300000, 0.400000), 1.0);
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb + lc, 1.0);
    }
...
void main(){
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(4.000000), 0.250000, 2.000000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(4.000000), 0.250000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.250000; }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec4 color_result = vec4(cosine_palette(sdf_result, vec3(0.020000, 0.010000, 0.050000), vec3(0.100000, 0.050000, 0.150000), vec3(1.000000, 0.800000, 0.600000), vec3(0.000000, 0.200000, 0.400000)), 1.0);
    ...
    // ── Layer 2: seed ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.600000, 0.200000), 1.0);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.250000) - 0.020000;
        float glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.400000, 0.900000), 1.0);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 5.000000 + time * 0.300000), cos(p.x * 5.000000 + time * 0.300000)) * 0.200000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.420000) - 0.005000;
        float glow_pulse = 1.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.300000, 0.300000, 0.400000), 1.0);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    }
...
`;
const UNIFORMS = [{name:'growth',default:0},{name:'pulse',default:0},{name:'breath',default:0}];
const PASS_WGSL_0 = `// Post-processing pass: focus
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'growth', target: 'seed', field: 'scale', weight: 0.5 },
      { source: 'growth', target: 'membrane', field: 'scale', weight: 0.8 },
      { source: 'growth', target: 'tendrils', field: 'intensity', weight: 0.6 },
      { source: 'pulse', target: 'seed', field: 'brightness', weight: 0.4 },
      { source: 'breath', target: 'membrane', field: 'brightness', weight: 0.3 },
      { source: 'breath', target: 'boundary', field: 'scale', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'growth', from: 0, to: 1, duration: 10, easing: 'ease_in_out' },
      { target: 'pulse', from: 0, to: 0.7, duration: 4, easing: 'ease_out' },
      { target: 'breath', from: 0, to: 0.4, duration: 6, easing: 'ease_in_out' },
    ];
  }
...
class Genesis extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-genesis: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get growth() { return this._renderer?.userParams['growth'] ?? 0; }
...
customElements.define('game-genesis', Genesis);
})();
