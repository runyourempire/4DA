...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: track ──
    {
        ...
        let sdf_result = abs(length(p) - 0.350000) - 0.010000;
        let glow_result = apply_glow(sdf_result, 0.800000);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(charcoal, 1.000000, 1.000000), 1.0);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_result = abs(length(p) - 0.350000) - 0.025000;
...
void main(){
    ...
    // ── Layer 0: track ──
    {
        ...
        float glow_result = apply_glow(sdf_result, 0.800000);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(charcoal, 1.000000, 1.000000), 1.0);
...
`;
const UNIFORMS = [{name:'charcoal',default:0},{name:'fill_angle',default:0},{name:'glow_val',default:2},{name:'gold',default:0}];
