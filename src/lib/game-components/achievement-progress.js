// game-achievement-progress.js — Generated by GAME compiler v0.2.0
// Zero-dependency WebGPU component with WebGL2 fallback.
//   <script type="module" src="./game-achievement-progress.js"></script>
//   <game-achievement-progress></game-achievement-progress>
...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    // ── Layer 0: track ──
    {
        ...
        // stage 0: ring(...)
        var sdf_result = abs(length(p) - 0.35) - 0.01;

        // stage 1: glow(...)
        let glow_result = apply_glow(sdf_result, 0.8);

        // stage 2: tint(...)
        var color_result = vec4f(vec3f(glow_result) * vec3f(0.15, 0.15, 0.17), 1.0);
...
void fs_main(input: VertexOutput){
    ...
    // ── Layer 0: track ──
    {
        ...
        // stage 0: ring(...)
        float sdf_result = abs(length(p) - 0.35) - 0.01;

        // stage 1: glow(...)
        float glow_result = apply_glow(sdf_result, 0.8);

        // stage 2: tint(...)
        vec4 color_result = vec4(vec3(glow_result) * vec3(0.15, 0.15, 0.17), 1.0);
...
class GameAchievementProgress extends HTMLElement {
  ...
  _frame() {
    ...
    this._paramValues[1] = (this._arcBaseOverrides?.[1] ?? 2) + (data_progress * 4.0);
  ...
  _initWebGL2() {
    ...
    this._glParamUniforms[0] = gl.getUniformLocation(prog, 'u_p_fill_angle');
    this._glParamUniforms[1] = gl.getUniformLocation(prog, 'u_p_glow_val');
...
customElements.define('game-achievement-progress', GameAchievementProgress);
export { GameAchievementProgress };
export default GameAchievementProgress;
