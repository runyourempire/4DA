// GAME Component: mandala — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: jewel ──
    {
        ...
        let sdf_b = sdf_box(p, 0.050000, 0.050000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.200000) - 0.015000;
        let glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.500000, 0.300000), 1.0);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_a = abs(length(p) - 0.300000) - 0.010000;
        let sdf_b = sdf_star(p, 6.000000, 0.320000, 0.280000);
        var sdf_result = smin(sdf_a, sdf_b, 0.040000);
        let glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.300000, 0.900000), 1.0);
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb + lc, 1.0);
    }
    ...
    // ── Layer 4: corona ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.600000, 0.800000), 1.0);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: edge ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.300000, 0.350000), 1.0);
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb + lc, 1.0);
    }
...
void main(){
    ...
    // ── Layer 1: jewel ──
    {
        ...
        float sdf_b = sdf_box(p, 0.050000, 0.050000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.200000) - 0.015000;
        float glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.500000, 0.300000), 1.0);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_a = abs(length(p) - 0.300000) - 0.010000;
        float sdf_b = sdf_star(p, 6.000000, 0.320000, 0.280000);
        float sdf_result = smin(sdf_a, sdf_b, 0.040000);
        float glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.300000, 0.900000), 1.0);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    }
    ...
    // ── Layer 4: corona ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.600000, 0.800000), 1.0);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    ...
    // ── Layer 5: edge ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.300000, 0.300000, 0.350000), 1.0);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    }
...
`;
const UNIFORMS = [{name:'bloom_amount',default:0},{name:'spin',default:0.2},{name:'breath',default:0}];
...
`;
const PASS_WGSL_1 = `// Post-processing pass: vignette
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'bloom_amount', target: 'jewel', field: 'brightness', weight: 0.5 },
      { source: 'bloom_amount', target: 'middle', field: 'brightness', weight: 0.3 },
      { source: 'spin', target: 'inner_ring', field: 'scale', weight: 0.2 },
      { source: 'breath', target: 'corona', field: 'intensity', weight: 0.4 },
      { source: 'breath', target: 'jewel', field: 'scale', weight: 0.15 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'bloom_amount', from: 0, to: 0.6, duration: 8, easing: 'ease_in_out' },
      { target: 'spin', from: 0.2, to: 0.6, duration: 6, easing: 'ease_out' },
      { target: 'breath', from: 0, to: 0.5, duration: 4, easing: 'ease_in_out' },
    ];
  }
...
class Mandala extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-mandala: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-mandala', Mandala);
})();
