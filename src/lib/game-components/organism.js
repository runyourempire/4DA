// GAME Component: organism — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.350000) - 0.020000;
        let glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.700000, 0.300000), 1.0);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = fbm2((p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.400000, 2.000000);
...
void main(){
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.350000) - 0.020000;
        float glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.700000, 0.300000), 1.0);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = fbm2((p * 5.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.400000, 2.000000);
...
`;
const UNIFORMS = [{name:'pulse',default:0},{name:'growth',default:0},{name:'breath',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'growth', target: 'nucleus', field: 'scale', weight: 0.4 },
      { source: 'growth', target: 'membrane', field: 'scale', weight: 0.6 },
      { source: 'pulse', target: 'cilia', field: 'intensity', weight: 0.5 },
      { source: 'pulse', target: 'nucleus', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'growth', from: 0, to: 1, duration: 8, easing: 'ease_in_out' },
      { target: 'pulse', from: 0, to: 0.6, duration: 3, easing: 'ease_out' },
    ];
  }
...
class Organism extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-organism: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-organism', Organism);
})();
