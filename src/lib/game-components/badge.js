// GAME Component: badge — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let sdf_a = abs(length(p) - 0.300000) - 0.080000;
    let sdf_b = sdf_star(p, 5.000000, 0.350000, 0.150000);
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.900000, 0.800000, 0.300000), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    float sdf_a = abs(length(p) - 0.300000) - 0.080000;
    float sdf_b = sdf_star(p, 5.000000, 0.350000, 0.150000);
    ...
    color_result = vec4(color_result.rgb * vec3(0.900000, 0.800000, 0.300000), 1.0);
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-badge: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-badge', Badge);
})();
