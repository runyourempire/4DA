// GAME Component: turbulence — auto-generated, do not edit.
(function(){
...
`;
const UNIFORMS = [{name:'storm',default:0},{name:'fury',default:0},{name:'lightning',default:0}];
const PASS_WGSL_0 = `// Post-processing pass: storm_glow
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'storm', target: 'storm_base', field: 'intensity', weight: 0.5 },
      { source: 'storm', target: 'undertow', field: 'intensity', weight: 0.4 },
      { source: 'fury', target: 'shear', field: 'intensity', weight: 0.7 },
      { source: 'fury', target: 'vortex', field: 'intensity', weight: 0.5 },
      { source: 'fury', target: 'embers', field: 'brightness', weight: 0.6 },
      { source: 'lightning', target: 'veins', field: 'brightness', weight: 0.8 },
      { source: 'lightning', target: 'shear', field: 'brightness', weight: 0.3 },
      { source: 'storm', target: 'vortex', field: 'scale', weight: 0.12 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'storm', from: 0, to: 1, duration: 8, easing: 'ease_in_out' },
      { target: 'fury', from: 0, to: 0.9, duration: 5, easing: 'ease_out' },
      { target: 'lightning', from: 0, to: 0.7, duration: 3, easing: 'ease_in_out' },
    ];
  }
...
class Turbulence extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-turbulence: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get storm() { return this._renderer?.userParams['storm'] ?? 0; }
  set storm(v) { this.setParam('storm', v); }
  get fury() { return this._renderer?.userParams['fury'] ?? 0; }
  set fury(v) { this.setParam('fury', v); }
  get lightning() { return this._renderer?.userParams['lightning'] ?? 0; }
  set lightning(v) { this.setParam('lightning', v); }
...
customElements.define('game-turbulence', Turbulence);
})();
