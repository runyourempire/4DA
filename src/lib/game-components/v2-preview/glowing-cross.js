// GAME Component: glowing-cross — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: field ──
    {
        ...
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, 0.300000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, 0.300000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.300000; }
        var sdf_result = noise2(p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.200000, 0.100000, 0.300000), vec3<f32>(0.200000, 0.100000, 0.300000), vec3<f32>(0.800000, 0.400000, 1.000000), vec3<f32>(0.000000, 0.100000, 0.200000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_a = length(p) - 0.2; // fallback
        let sdf_b = sdf_star(p, 4.000000, 0.200000, 0.080000);
        var sdf_result = smin(sdf_a, sdf_b, 0.050000);
        let glow_pulse = 3.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.300000, 0.300000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_a = abs(length(p) - 0.220000) - 0.025000;
        let sdf_b = length(p) - 0.2; // fallback
        var sdf_result = max(sdf_a, -sdf_b);
        let glow_pulse = 2.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.500000, 0.200000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 4: rings ──
    {
        ...
        for (var onion_i: i32 = 0; onion_i < i32(4.000000); onion_i = onion_i + 1) { sdf_result = abs(sdf_result) - 0.008000; }
        let glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.200000, 0.400000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: rays ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.600000, 0.300000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 6.000000 + time * 0.500000), cos(p.x * 6.000000 + time * 0.500000)) * 0.200000;
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.300000, 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.300000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.300000; }
        var sdf_result = fbm2((p * 14.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(2.000000), 0.500000, 2.000000);
        let glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.800000, 0.500000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
uniform float u_p_power;
uniform float u_p_breath;
uniform float u_p_sanctity;
...
void main(){
    ...
    float power = u_p_power;
    float breath = u_p_breath;
    float sanctity = u_p_sanctity;
    ...
    // ── Layer 1: field ──
    {
        ...
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(5.000000), 0.500000, 0.300000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(5.000000), 0.500000, 0.300000);
        p = p + vec2(warp_x, warp_y) * 0.300000; }
        float sdf_result = noise2(p * 3.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.200000, 0.100000, 0.300000), vec3(0.200000, 0.100000, 0.300000), vec3(0.800000, 0.400000, 1.000000), vec3(0.000000, 0.100000, 0.200000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_a = length(p) - 0.2; // fallback
        float sdf_b = sdf_star(p, 4.000000, 0.200000, 0.080000);
        float sdf_result = smin(sdf_a, sdf_b, 0.050000);
        float glow_pulse = 3.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.300000, 0.300000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_a = abs(length(p) - 0.220000) - 0.025000;
        float sdf_b = length(p) - 0.2; // fallback
        float sdf_result = max(sdf_a, -sdf_b);
        float glow_pulse = 2.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.500000, 0.200000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 4: rings ──
    {
        ...
        for (int onion_i = 0; onion_i < int(4.000000); onion_i++) { sdf_result = abs(sdf_result) - 0.008000; }
        float glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.200000, 0.400000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 5: rays ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.600000, 0.300000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 6.000000 + time * 0.500000), cos(p.x * 6.000000 + time * 0.500000)) * 0.200000;
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(3.000000), 0.300000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(3.000000), 0.300000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.300000; }
        float sdf_result = fbm2((p * 14.000000 + vec2(time * 0.1, time * 0.07)), int(2.000000), 0.500000, 2.000000);
        float glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.800000, 0.500000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.750000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
`;
const UNIFORMS = [{name:'power',default:0},{name:'breath',default:0},{name:'sanctity',default:0}];
const PASS_WGSL_0 = `// Post-processing pass: radiance
...
`;
const PASS_WGSL_1 = `// Post-processing pass: focus
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'power', target: 'cross_core', field: 'brightness', weight: 0.6 },
      { source: 'power', target: 'inner_halo', field: 'brightness', weight: 0.5 },
      { source: 'power', target: 'rings', field: 'brightness', weight: 0.4 },
      { source: 'breath', target: 'cross_core', field: 'scale', weight: 0.12 },
      { source: 'breath', target: 'inner_halo', field: 'scale', weight: 0.08 },
      { source: 'sanctity', target: 'rays', field: 'brightness', weight: 0.5 },
      { source: 'sanctity', target: 'dust', field: 'brightness', weight: 0.4 },
      { source: 'sanctity', target: 'field', field: 'intensity', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'power', from: 0, to: 1, duration: 6, easing: 'ease_in_out' },
      { target: 'breath', from: 0, to: 0.7, duration: 4, easing: 'ease_in_out' },
      { target: 'sanctity', from: 0, to: 0.8, duration: 10, easing: 'ease_out' },
    ];
  }
...
class GlowingCross extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-glowing-cross: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get power() { return this._renderer?.userParams['power'] ?? 0; }
  set power(v) { this.setParam('power', v); }
  get breath() { return this._renderer?.userParams['breath'] ?? 0; }
  set breath(v) { this.setParam('breath', v); }
  get sanctity() { return this._renderer?.userParams['sanctity'] ?? 0; }
  set sanctity(v) { this.setParam('sanctity', v); }
...
customElements.define('game-glowing-cross', GlowingCross);
})();
