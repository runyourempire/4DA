// GAME Component: living-field — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: ground ──
    {
        ...
        { let warp_x = fbm2(p * 1.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, 0.300000);
        let warp_y = fbm2(p * 1.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, 0.300000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.400000, 0.300000), vec3<f32>(0.300000, 0.300000, 0.200000), vec3<f32>(0.800000, 0.600000, 0.300000), vec3<f32>(0.100000, 0.150000, 0.050000));
    ...
    // ── Layer 3: trails ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.500000, 0.200000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: warmth_glow ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.400000, 0.200000), vec3<f32>(0.400000, 0.300000, 0.200000), vec3<f32>(0.800000, 0.500000, 0.200000), vec3<f32>(0.000000, 0.100000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 6: fireflies ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 1.000000, 0.700000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 1: ground ──
    {
        ...
        { float warp_x = fbm2(p * 1.000000 + vec2(0.0, 1.3), int(5.000000), 0.500000, 0.300000);
        float warp_y = fbm2(p * 1.000000 + vec2(1.7, 0.0), int(5.000000), 0.500000, 0.300000);
        p = p + vec2(warp_x, warp_y) * 0.300000; }
        float sdf_result = fbm2((p * 2.000000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.400000, 0.300000), vec3(0.300000, 0.300000, 0.200000), vec3(0.800000, 0.600000, 0.300000), vec3(0.100000, 0.150000, 0.050000));
    ...
    // ── Layer 3: trails ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.500000, 0.200000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.960000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 5: warmth_glow ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.400000, 0.200000), vec3(0.400000, 0.300000, 0.200000), vec3(0.800000, 0.500000, 0.200000), vec3(0.000000, 0.100000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 6: fireflies ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 1.000000, 0.700000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.720000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
`;
const UNIFORMS = [{name:'life',default:0},{name:'warmth',default:0},{name:'density',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'life', target: 'organisms', field: 'brightness', weight: 0.6 },
      { source: 'life', target: 'trails', field: 'intensity', weight: 0.4 },
      { source: 'life', target: 'fireflies', field: 'brightness', weight: 0.5 },
      { source: 'warmth', target: 'ground', field: 'brightness', weight: 0.3 },
      { source: 'warmth', target: 'warmth_glow', field: 'brightness', weight: 0.5 },
      { source: 'density', target: 'pollen', field: 'brightness', weight: 0.4 },
      { source: 'density', target: 'organisms', field: 'scale', weight: 0.1 },
      { source: 'life', target: 'pollen', field: 'intensity', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      ...
      { target: 'density', from: 0, to: 0.7, duration: 10, easing: 'ease_in_out' },
    ];
  }
...
class LivingField extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-living-field: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get life() { return this._renderer?.userParams['life'] ?? 0; }
  set life(v) { this.setParam('life', v); }
  get warmth() { return this._renderer?.userParams['warmth'] ?? 0; }
  set warmth(v) { this.setParam('warmth', v); }
  get density() { return this._renderer?.userParams['density'] ?? 0; }
  set density(v) { this.setParam('density', v); }
...
customElements.define('game-living-field', LivingField);
})();
