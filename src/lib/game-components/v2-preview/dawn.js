...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let emergence = u.p_emergence;
    let warmth = u.p_warmth;
    let sky_glow = u.p_sky_glow;
    ...
    // ── Layer 1: sky ──
    {
        ...
        let sdf_result = smoothstep(0.000000, 0.900000, length(p));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.300000, 0.200000), vec3<f32>(0.500000, 0.300000, 0.400000), vec3<f32>(1.000000, 0.800000, 0.500000), vec3<f32>(0.000000, 0.100000, 0.300000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.550000, 0.350000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.550000, 0.350000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.400000, 0.350000), vec3<f32>(0.500000, 0.300000, 0.250000), vec3<f32>(1.000000, 0.800000, 0.500000), vec3<f32>(0.000000, 0.150000, 0.250000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 6.000000 + time * 0.400000), cos(p.x * 6.000000 + time * 0.400000)) * 0.150000;
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.700000, 0.500000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 4: sun ──
    {
        ...
        let sdf_b = sdf_star(p, 6.000000, 0.090000, 0.040000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 0.300000), cos(p.x * 3.000000 + time * 0.300000)) * 0.100000;
        var sdf_result = abs(length(p) - 0.140000) - 0.012000;
        let glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.600000, 0.200000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 6: god_rays ──
    {
        ...
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.200000), cos(p.x * 2.000000 + time * 0.200000)) * 0.120000;
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.850000, 0.500000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 7: horizon_glow ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_result = smoothstep(0.100000, 0.600000, length(p));
...
uniform float u_p_emergence;
uniform float u_p_warmth;
uniform float u_p_sky_glow;
...
void main(){
    ...
    float emergence = u_p_emergence;
    float warmth = u_p_warmth;
    float sky_glow = u_p_sky_glow;
    ...
    // ── Layer 1: sky ──
    {
        ...
        float sdf_result = smoothstep(0.000000, 0.900000, length(p));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.300000, 0.200000), vec3(0.500000, 0.300000, 0.400000), vec3(1.000000, 0.800000, 0.500000), vec3(0.000000, 0.100000, 0.300000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(6.000000), 0.550000, 0.350000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(6.000000), 0.550000, 0.350000);
        p = p + vec2(warp_x, warp_y) * 0.350000; }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.400000, 0.350000), vec3(0.500000, 0.300000, 0.250000), vec3(1.000000, 0.800000, 0.500000), vec3(0.000000, 0.150000, 0.250000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 6.000000 + time * 0.400000), cos(p.x * 6.000000 + time * 0.400000)) * 0.150000;
        ...
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.700000, 0.500000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.840000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_a = sdf_circle(p, 0.060000);
        float sdf_b = sdf_star(p, 6.000000, 0.090000, 0.040000);
        float sdf_result = smin(sdf_a, sdf_b, 0.030000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 3.000000 + time * 0.300000), cos(p.x * 3.000000 + time * 0.300000)) * 0.100000;
        float sdf_result = abs(length(p) - 0.140000) - 0.012000;
        float glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.600000, 0.200000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.870000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 6: god_rays ──
    {
        ...
        p = p + vec2(sin(p.y * 2.000000 + time * 0.200000), cos(p.x * 2.000000 + time * 0.200000)) * 0.120000;
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.850000, 0.500000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = smoothstep(0.100000, 0.600000, length(p));
...
`;
const UNIFORMS = [{name:'emergence',default:0},{name:'warmth',default:0},{name:'sky_glow',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'emergence', target: 'sun', field: 'brightness', weight: 0.6 },
      { source: 'emergence', target: 'corona', field: 'brightness', weight: 0.5 },
      { source: 'emergence', target: 'god_rays', field: 'brightness', weight: 0.4 },
      { source: 'warmth', target: 'clouds', field: 'brightness', weight: 0.3 },
      { source: 'warmth', target: 'horizon_glow', field: 'brightness', weight: 0.5 },
      { source: 'warmth', target: 'wisps', field: 'brightness', weight: 0.3 },
      { source: 'sky_glow', target: 'sky', field: 'brightness', weight: 0.2 },
      { source: 'sky_glow', target: 'clouds', field: 'intensity', weight: 0.3 },
      { source: 'emergence', target: 'sun', field: 'scale', weight: 0.15 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'emergence', from: 0, to: 1, duration: 12, easing: 'ease_in_out' },
      { target: 'warmth', from: 0, to: 0.8, duration: 8, easing: 'ease_out' },
      { target: 'sky_glow', from: 0, to: 0.9, duration: 10, easing: 'ease_in_out' },
    ];
  }
...
class Dawn extends HTMLElement {
  ...
  get emergence() { return this._renderer?.userParams['emergence'] ?? 0; }
  set emergence(v) { this.setParam('emergence', v); }
  get warmth() { return this._renderer?.userParams['warmth'] ?? 0; }
  set warmth(v) { this.setParam('warmth', v); }
  get sky_glow() { return this._renderer?.userParams['sky_glow'] ?? 0; }
  set sky_glow(v) { this.setParam('sky_glow', v); }
