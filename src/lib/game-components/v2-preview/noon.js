...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: atmosphere ──
    {
        ...
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.550000, 0.400000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.550000, 0.400000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.400000; }
        var sdf_result = noise2(p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.500000, 0.500000), vec3<f32>(0.500000, 0.500000, 0.500000), vec3<f32>(1.000000, 0.700000, 0.400000), vec3<f32>(0.000000, 0.150000, 0.200000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 6.000000 + time * 0.800000), cos(p.x * 6.000000 + time * 0.800000)) * 0.200000;
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.250000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.250000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.250000; }
        var sdf_result = fbm2((p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.500000, 2.000000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_a = sdf_circle(p, 0.080000);
        let sdf_b = sdf_star(p, 8.000000, 0.120000, 0.060000);
        var sdf_result = smin(sdf_a, sdf_b, 0.040000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 4.000000 + time * 0.300000), cos(p.x * 4.000000 + time * 0.300000)) * 0.100000;
        var sdf_result = abs(length(p) - 0.180000) - 0.015000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_result = smoothstep(0.300000, 0.900000, length(p));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.400000, 0.300000), vec3<f32>(0.500000, 0.400000, 0.300000), vec3<f32>(0.800000, 0.700000, 0.400000), vec3<f32>(0.200000, 0.100000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
...
`;
const UNIFORMS = [{name:'intensity',default:0},{name:'shimmer',default:0},{name:'radiance',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'intensity', target: 'sun', field: 'brightness', weight: 0.5 },
      { source: 'intensity', target: 'corona', field: 'brightness', weight: 0.4 },
      { source: 'shimmer', target: 'shimmer_field', field: 'intensity', weight: 0.6 },
      { source: 'shimmer', target: 'atmosphere', field: 'intensity', weight: 0.3 },
      { source: 'radiance', target: 'rays', field: 'brightness', weight: 0.5 },
      { source: 'radiance', target: 'sun', field: 'scale', weight: 0.15 },
      { source: 'intensity', target: 'haze', field: 'brightness', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'intensity', from: 0, to: 1, duration: 6, easing: 'ease_in_out' },
      { target: 'shimmer', from: 0, to: 0.7, duration: 4, easing: 'ease_out' },
      { target: 'radiance', from: 0, to: 0.9, duration: 8, easing: 'ease_in_out' },
    ];
  }
...
class Noon extends HTMLElement {
  ...
  get shimmer() { return this._renderer?.userParams['shimmer'] ?? 0; }
  set shimmer(v) { this.setParam('shimmer', v); }
  get radiance() { return this._renderer?.userParams['radiance'] ?? 0; }
  set radiance(v) { this.setParam('radiance', v); }
