// GAME Component: organic-flow — auto-generated, do not edit.
(function(){
...
`;
const UNIFORMS = [{name:'current',default:0},{name:'luminance',default:0},{name:'depth',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'current', target: 'deep_current', field: 'intensity', weight: 0.4 },
      { source: 'current', target: 'mid_flow', field: 'intensity', weight: 0.5 },
      { source: 'current', target: 'surface', field: 'intensity', weight: 0.6 },
      { source: 'luminance', target: 'filaments', field: 'brightness', weight: 0.6 },
      { source: 'luminance', target: 'sparkles', field: 'brightness', weight: 0.5 },
      { source: 'depth', target: 'deep_current', field: 'scale', weight: 0.15 },
      { source: 'depth', target: 'mid_flow', field: 'scale', weight: 0.1 },
      { source: 'current', target: 'filaments', field: 'scale', weight: 0.08 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'current', from: 0, to: 1, duration: 10, easing: 'ease_in_out' },
      { target: 'luminance', from: 0, to: 0.8, duration: 6, easing: 'ease_out' },
      { target: 'depth', from: 0, to: 0.7, duration: 8, easing: 'ease_in_out' },
    ];
  }
...
class OrganicFlow extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-organic-flow: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get current() { return this._renderer?.userParams['current'] ?? 0; }
  set current(v) { this.setParam('current', v); }
  get luminance() { return this._renderer?.userParams['luminance'] ?? 0; }
  set luminance(v) { this.setParam('luminance', v); }
...
customElements.define('game-organic-flow', OrganicFlow);
})();
