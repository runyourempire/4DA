// GAME Component: neon-cells — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let pulse = u.p_pulse;
    let division = u.p_division;
    let glow_energy = u.p_glow_energy;
    ...
    // ── Layer 1: substrate ──
    {
        ...
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, 0.350000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, 0.350000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.350000; }
        var sdf_result = voronoi2(p * 4.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.500000, 0.500000), vec3<f32>(0.500000, 0.500000, 0.500000), vec3<f32>(1.000000, 1.000000, 1.000000), vec3<f32>(0.000000, 0.100000, 0.200000));
    ...
    // ── Layer 2: cells ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.500000, 0.500000), vec3<f32>(0.500000, 0.500000, 0.500000), vec3<f32>(2.000000, 1.000000, 0.000000), vec3<f32>(0.500000, 0.200000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.200000, 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.200000, 2.000000);
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.800000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 4: nuclei ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.400000, 0.800000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: circulation ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.300000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.300000, 2.000000);
...
uniform float u_p_division;
uniform float u_p_glow_energy;
...
void main(){
    ...
    float division = u_p_division;
    float glow_energy = u_p_glow_energy;
    ...
    // ── Layer 1: substrate ──
    {
        ...
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(5.000000), 0.500000, 0.350000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(5.000000), 0.500000, 0.350000);
        p = p + vec2(warp_x, warp_y) * 0.350000; }
        float sdf_result = voronoi2(p * 4.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.500000, 0.500000), vec3(0.500000, 0.500000, 0.500000), vec3(1.000000, 1.000000, 1.000000), vec3(0.000000, 0.100000, 0.200000));
    ...
    // ── Layer 2: cells ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.500000, 0.500000), vec3(0.500000, 0.500000, 0.500000), vec3(2.000000, 1.000000, 0.000000), vec3(0.500000, 0.200000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(3.000000), 0.200000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(3.000000), 0.200000, 2.000000);
        ...
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.800000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 3.000000 + time * 0.300000), cos(p.x * 3.000000 + time * 0.300000)) * 0.120000;
        float sdf_result = sdf_circle(p, 0.020000);
        float glow_pulse = 3.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.400000, 0.800000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 5: circulation ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.300000, 2.000000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.300000, 2.000000);
...
`;
const UNIFORMS = [{name:'pulse',default:0},{name:'division',default:0},{name:'glow_energy',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'pulse', target: 'cells', field: 'brightness', weight: 0.5 },
      { source: 'pulse', target: 'nuclei', field: 'brightness', weight: 0.7 },
      { source: 'division', target: 'membrane', field: 'intensity', weight: 0.4 },
      { source: 'division', target: 'substrate', field: 'intensity', weight: 0.3 },
      { source: 'glow_energy', target: 'circulation', field: 'brightness', weight: 0.6 },
      { source: 'glow_energy', target: 'cells', field: 'scale', weight: 0.1 },
      { source: 'pulse', target: 'membrane', field: 'scale', weight: 0.08 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'pulse', from: 0, to: 0.8, duration: 5, easing: 'ease_in_out' },
      { target: 'division', from: 0, to: 0.6, duration: 8, easing: 'ease_out' },
      { target: 'glow_energy', from: 0, to: 1, duration: 3, easing: 'ease_in_out' },
    ];
  }
...
class NeonCells extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-neon-cells: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get division() { return this._renderer?.userParams['division'] ?? 0; }
  set division(v) { this.setParam('division', v); }
  get glow_energy() { return this._renderer?.userParams['glow_energy'] ?? 0; }
  set glow_energy(v) { this.setParam('glow_energy', v); }
...
customElements.define('game-neon-cells', NeonCells);
})();
