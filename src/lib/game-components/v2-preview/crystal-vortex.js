// GAME Component: crystal-vortex — auto-generated, do not edit.
(function(){
...
void main(){
    ...
    // ── Layer 1: vortex_core ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(5.000000), 0.550000, 0.350000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(5.000000), 0.550000, 0.350000);
        p = p + vec2(warp_x, warp_y) * 0.350000; }
        float sdf_result = voronoi2(p * 6.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.500000, 0.500000), vec3(0.500000, 0.500000, 0.500000), vec3(1.000000, 1.000000, 0.500000), vec3(0.800000, 0.900000, 0.300000));
    ...
    // ── Layer 2: facets ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.500000, 0.500000), vec3(0.500000, 0.500000, 0.500000), vec3(2.000000, 1.000000, 0.500000), vec3(0.000000, 0.250000, 0.500000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
        float la = color_result.a;
    ...
    // ── Layer 4: eye ──
    {
        ...
        float sdf_b = abs(length(p) - 0.100000) - 0.010000;
    ...
    // ── Layer 5: spirals ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = vec2(length(p), atan(p.y, p.x));
        p = p + vec2(sin(p.y * 2.000000 + time * 0.400000), cos(p.x * 2.000000 + time * 0.400000)) * 0.200000;
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(4.000000), 0.300000, 2.000000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(4.000000), 0.300000, 2.000000);
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.900000, 0.500000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 6: outer_ring ──
    {
        ...
        float sdf_result = abs(length(p) - 0.420000) - 0.015000;
        float glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);
...
`;
const UNIFORMS = [{name:'spin',default:0},{name:'refraction',default:0},{name:'depth',default:0}];
const PASS_WGSL_0 = `// Post-processing pass: shimmer
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'spin', target: 'vortex_core', field: 'intensity', weight: 0.4 },
      { source: 'spin', target: 'spirals', field: 'intensity', weight: 0.6 },
      { source: 'spin', target: 'facets', field: 'intensity', weight: 0.3 },
      { source: 'refraction', target: 'prismatic', field: 'brightness', weight: 0.6 },
      { source: 'refraction', target: 'facets', field: 'brightness', weight: 0.4 },
      { source: 'depth', target: 'eye', field: 'brightness', weight: 0.5 },
      { source: 'depth', target: 'vortex_core', field: 'scale', weight: 0.1 },
      { source: 'depth', target: 'outer_ring', field: 'brightness', weight: 0.3 },
      { source: 'spin', target: 'eye', field: 'scale', weight: 0.08 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'spin', from: 0, to: 0.8, duration: 10, easing: 'ease_in_out' },
      { target: 'refraction', from: 0, to: 0.7, duration: 6, easing: 'ease_out' },
      { target: 'depth', from: 0, to: 1, duration: 8, easing: 'ease_in_out' },
    ];
  }
...
class CrystalVortex extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-crystal-vortex: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get spin() { return this._renderer?.userParams['spin'] ?? 0; }
  set spin(v) { this.setParam('spin', v); }
  get refraction() { return this._renderer?.userParams['refraction'] ?? 0; }
...
customElements.define('game-crystal-vortex', CrystalVortex);
})();
