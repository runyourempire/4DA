...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let relevance_avg = u.p_relevance_avg;
    ...
    // ── Layer 0: base ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.150000, 0.150000, 0.150000), vec3<f32>(0.250000, 0.250000, 0.250000), vec3<f32>(1.000000, 1.000000, 1.000000), vec3<f32>(0.300000, 0.000000, 0.050000));
    ...
    // ── Layer 1: pulse ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(signal_count, signal_count, signal_count), vec3<f32>(0.300000, 0.300000, 0.400000), vec3<f32>(1.000000, 0.800000, 0.600000), vec3<f32>(0.300000, 0.000000, 0.080000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.500000) - 0.006000;
        let glow_pulse = relevance_avg * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.250000, 0.920000, 0.800000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
uniform float u_p_relevance_avg;
...
void main(){
    ...
    float relevance_avg = u_p_relevance_avg;
    ...
    // ── Layer 0: base ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.150000, 0.150000, 0.150000), vec3(0.250000, 0.250000, 0.250000), vec3(1.000000, 1.000000, 1.000000), vec3(0.300000, 0.000000, 0.050000));
    ...
    // ── Layer 1: pulse ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(signal_count, signal_count, signal_count), vec3(0.300000, 0.300000, 0.400000), vec3(1.000000, 0.800000, 0.600000), vec3(0.300000, 0.000000, 0.080000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.500000) - 0.006000;
        float glow_pulse = relevance_avg * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.250000, 0.920000, 0.800000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'signal_count',default:0},{name:'relevance_avg',default:0},{name:'freshness',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'signal_count', target: 'accent', field: 'brightness', weight: 0.4 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class SignalWaveform extends HTMLElement {
  ...
  get signal_count() { return this._renderer?.userParams['signal_count'] ?? 0; }
  set signal_count(v) { this.setParam('signal_count', v); }
  get relevance_avg() { return this._renderer?.userParams['relevance_avg'] ?? 0; }
  set relevance_avg(v) { this.setParam('relevance_avg', v); }
