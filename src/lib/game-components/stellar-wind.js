// GAME Component: stellar-wind — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.010000, 0.000000, 0.020000), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.010000, 0.000000, 0.020000), 1.0);
...
class GameFlowField {
  ...
  dispatch(dt) {
    ...
    f[0] = 4; f[1] = 0.4; f[2] = 1.5; f[3] = this._time;
...
class StellarWind extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-stellar-wind: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    if (this._renderer.device) {
      ...
      this._renderer._preRender = () => {
        const dt = 1/60;
        if (this._flowSim) this._flowSim.dispatch(dt);
      };
    }
...
customElements.define('game-stellar-wind', StellarWind);
})();
