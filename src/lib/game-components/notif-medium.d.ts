...
/**
 * `<game-notif-medium>` Web Component
 *
 * A self-contained WebGPU/WebGL2 shader component.
 *
 * @example
 * ```html
 * <game-notif-medium intensity="0" hover="0"></game-notif-medium>
 * ```
 *
 * @example
 * ```typescript
 * const el = document.querySelector('game-notif-medium')!;
   * el.intensity = 0;
   * el.hover = 0;
 * ```
 */
interface GameNotifMediumElement extends HTMLElement {
...
declare global {
  interface HTMLElementTagNameMap {
    'game-notif-medium': GameNotifMediumElement;
  }
}
