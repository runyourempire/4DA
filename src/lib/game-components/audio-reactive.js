// GAME Component: audio-reactive — auto-generated, do not edit.
(function(){
...
class GameListenPipeline {
  constructor(analyser) {
    this._analyser = analyser;
    this._fftData = new Float32Array(analyser.frequencyBinCount);
    this._timeData = new Float32Array(analyser.fftSize);
    this._prevEnergy = 0;
    this._prevSpectrum = null;
    this.signals = {};
    this.signals['onset'] = 0;
    this.signals['melody'] = 0;
  ...
  update() {
    ...
    { // onset: onset
      ...
      for (let i = 0; i < this._fftData.length; i++) {
        const v = (this._fftData[i] + 140) / 140;
        energy += v * v;
      }
      energy /= this._fftData.length;
      if (this._prevSpectrum !== null && energy - this._prevEnergy > 0.7) {
        this.signals['onset'] = 1.0;
      } else {
        this.signals['onset'] = Math.max(0, this.signals['onset'] - 0.055566666666666674);
      }
      this._prevEnergy = energy;
      this._prevSpectrum = true;
    }
    { // pitch: melody
      let bestCorr = 0, bestLag = 0;
      const sr = this._analyser.context.sampleRate;
      const minLag = Math.floor(sr / 4000);
      const maxLag = Math.ceil(sr / 200);
      for (let lag = minLag; lag <= Math.min(maxLag, this._timeData.length / 2); lag++) {
        let corr = 0;
        for (let i = 0; i < this._timeData.length - lag; i++) {
          corr += this._timeData[i] * this._timeData[i + lag];
        }
        if (corr > bestCorr) { bestCorr = corr; bestLag = lag; }
      }
      this.signals['melody'] = bestLag > 0 ? (sr / bestLag - 200) / (4000 - 200) : 0;
    }
    { // rhythm phase: rhythm
      let rmsEnergy = 0;
      for (let i = 0; i < this._timeData.length; i++) {
        rmsEnergy += this._timeData[i] * this._timeData[i];
      }
      rmsEnergy = Math.sqrt(rmsEnergy / this._timeData.length);
      this.signals['rhythm'] = (rmsEnergy * 16) % 1.0;
    }
    return this.signals;
  }
}
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-audio-reactive: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-audio-reactive', AudioReactive);
})();
