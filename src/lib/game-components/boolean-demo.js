// GAME Component: boolean-demo — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_a = sdf_circle(p, 0.200000);
        let sdf_b = sdf_circle(p, 0.200000);
        var sdf_result = smin(sdf_a, sdf_b, blend_k);
...
void main(){
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_a = sdf_circle(p, 0.200000);
        float sdf_b = sdf_circle(p, 0.200000);
        float sdf_result = smin(sdf_a, sdf_b, blend_k);
...
`;
const UNIFORMS = [{name:'blend_k',default:0.1}];
...
class BooleanDemo extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-boolean-demo: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get blend_k() { return this._renderer?.userParams['blend_k'] ?? 0; }
  set blend_k(v) { this.setParam('blend_k', v); }
...
customElements.define('game-boolean-demo', BooleanDemo);
})();
