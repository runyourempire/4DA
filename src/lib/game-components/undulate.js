// GAME Component: undulate — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.150000, 2.000000);
    let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.150000, 2.000000);
    p = p + vec2<f32>(warp_x, warp_y) * 0.150000; }
    let sdf_result = smoothstep(0.000000, 0.800000, length(p));
    var color_result = vec4<f32>(cosine_palette(sdf_result, vec3<f32>(0.500000, 0.500000, 0.500000), vec3<f32>(0.500000, 0.500000, 0.500000), vec3<f32>(2.000000, 1.000000, 0.000000), vec3<f32>(0.500000, 0.200000, 0.250000)), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(3.000000), 0.150000, 2.000000);
    float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(3.000000), 0.150000, 2.000000);
    p = p + vec2(warp_x, warp_y) * 0.150000; }
    float sdf_result = smoothstep(0.000000, 0.800000, length(p));
    vec4 color_result = vec4(cosine_palette(sdf_result, vec3(0.500000, 0.500000, 0.500000), vec3(0.500000, 0.500000, 0.500000), vec3(2.000000, 1.000000, 0.000000), vec3(0.500000, 0.200000, 0.250000)), 1.0);
...
class Undulate extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-undulate: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-undulate', Undulate);
})();
