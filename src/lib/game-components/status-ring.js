// GAME Component: status-ring — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: glow_core ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.800000, 0.200000), 1.0);
    ...
    // ── Layer 1: outer_ring ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.700000, 1.000000), 1.0);
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb + lc, 1.0);
    }
...
void main(){
    ...
    // ── Layer 0: glow_core ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.800000, 0.200000), 1.0);
    ...
    // ── Layer 1: outer_ring ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.700000, 1.000000), 1.0);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    }
...
class StatusRing extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-status-ring: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-status-ring', StatusRing);
})();
