...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: field ──
    {
        ...
        { let warp_x = fbm2(p * 1.800000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.120000, 2.000000);
        let warp_y = fbm2(p * 1.800000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.120000, 2.000000);
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.200000, 0.100000), color_result.a);
    ...
    // ── Layer 2: pulse ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.950000, 0.400000, 0.100000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.480000) - 0.004000;
        let glow_pulse = 0.600000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.120000, 0.080000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 1: field ──
    {
        ...
        { float warp_x = fbm2(p * 1.800000 + vec2(0.0, 1.3), int(2.000000), 0.120000, 2.000000);
        float warp_y = fbm2(p * 1.800000 + vec2(1.7, 0.0), int(2.000000), 0.120000, 2.000000);
        ...
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.200000, 0.100000), color_result.a);
    ...
    // ── Layer 2: pulse ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.950000, 0.400000, 0.100000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.480000) - 0.004000;
        float glow_pulse = 0.600000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.120000, 0.080000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'intensity', target: 'field', field: 'brightness', weight: 0.8 },
      { source: 'intensity', target: 'pulse', field: 'brightness', weight: 0.6 },
      { source: 'hover', target: 'field', field: 'brightness', weight: 0.15 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
