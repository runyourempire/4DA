...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: field ──
    {
        ...
        { let warp_x = fbm2(p * 1.800000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.100000, 2.000000);
        let warp_y = fbm2(p * 1.800000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.100000, 2.000000);
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.150000, 0.080000), color_result.a);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.380000) - 0.025000;
        let glow_pulse = 1.800000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.700000, 0.250000, 0.080000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 1: field ──
    {
        ...
        { float warp_x = fbm2(p * 1.800000 + vec2(0.0, 1.3), int(2.000000), 0.100000, 2.000000);
        float warp_y = fbm2(p * 1.800000 + vec2(1.7, 0.0), int(2.000000), 0.100000, 2.000000);
        ...
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.150000, 0.080000), color_result.a);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.380000) - 0.025000;
        float glow_pulse = 1.800000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.700000, 0.250000, 0.080000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'intensity', target: 'field', field: 'brightness', weight: 0.7 },
      { source: 'hover', target: 'field', field: 'brightness', weight: 0.12 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
