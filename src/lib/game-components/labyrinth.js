...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.030000, 0.010000, 0.020000), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.030000, 0.010000, 0.020000), 1.0);
...
class GameReactionField {
  ...
  async init() {
    ...
    // Seed: random field, density 0.05
    for (let i = 0; i < cellCount; i++) {
      if (Math.random() < 0.05) init[i * 2 + 1] = 1.0;
    }
