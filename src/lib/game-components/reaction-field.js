...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.010000, 0.010000, 0.030000), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.010000, 0.010000, 0.030000), 1.0);
...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let abb = a * b * b;
    let new_a = a + (1 * lap.x - abb + 0.037 * (1.0 - a));
    let new_b = b + (0.5 * lap.y + abb - (0.037 + 0.06) * b);
