...
/**
 * `<game-notif-critical>` Web Component
 *
 * A self-contained WebGPU/WebGL2 shader component.
 *
 * @example
 * ```html
 * <game-notif-critical intensity="0" hover="0"></game-notif-critical>
 * ```
 *
 * @example
 * ```typescript
 * const el = document.querySelector('game-notif-critical')!;
   * el.intensity = 0;
   * el.hover = 0;
 * ```
 */
interface GameNotifCriticalElement extends HTMLElement {
...
declare global {
  interface HTMLElementTagNameMap {
    'game-notif-critical': GameNotifCriticalElement;
  }
}
