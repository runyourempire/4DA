// GAME Component: turing-patterns — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.020000, 0.020000, 0.050000), 1.0);
    return color_result;
}
`;
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.020000, 0.020000, 0.050000), 1.0);
...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let abb = a * b * b;
    let new_a = a + (1 * lap.x - abb + 0.055 * (1.0 - a));
    let new_b = b + (0.5 * lap.y + abb - (0.055 + 0.062) * b);
...
class GameReactionField {
  ...
  async init() {
    ...
    // Seed: center blob, radius 0.15
    const cx = this._w / 2, cy = this._h / 2;
    const r = Math.floor(Math.max(this._w, this._h) * 0.15);
    for (let dy = -r; dy <= r; dy++) {
      for (let dx = -r; dx <= r; dx++) {
        if (dx*dx + dy*dy <= r*r) {
          const idx = ((cy + dy) * this._w + (cx + dx)) * 2;
          if (idx >= 0 && idx < init.length - 1) {
            init[idx + 1] = 1.0;
          }
        }
      }
...
class TuringPatterns extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-turing-patterns: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    if (this._renderer.device) {
      ...
      this._renderer._preRender = () => {
        const dt = 1/60;
        if (this._reactSim) this._reactSim.dispatch(4);
      };
    }
...
customElements.define('game-turing-patterns', TuringPatterns);
})();
