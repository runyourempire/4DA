// GAME Component: neon-heart — auto-generated, do not edit.
(function(){
...
fn sdf_heart(p: vec2<f32>, sz: f32) -> f32 {
    let q = vec2<f32>(abs(p.x), p.y);
    let b = vec2<f32>(sz * 0.5, sz * 0.8);
    let r = 0.5 * (b.x + b.y);
    let d = length(q - vec2<f32>(0.0, r * 0.5)) - r;
    let a = atan2(q.x, q.y - r * 0.5);
    let h = sz * (0.5 + 0.3 * cos(a));
    return length(q - vec2<f32>(0.0, r * 0.5)) - h;
}
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var sdf_result = sdf_heart(p, 0.250000);
    let glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
    let glow_result = apply_glow(sdf_result, glow_pulse);
    var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.200000, 0.400000), 1.0);
    return color_result;
}
`;
...
float sdf_heart(vec2 p, float sz){
    vec2 q = vec2(abs(p.x), p.y);
    vec2 b = vec2(sz * 0.5, sz * 0.8);
    float r = 0.5 * (b.x + b.y);
    float d = length(q - vec2(0.0, r * 0.5)) - r;
    float a = atan(q.x, q.y - r * 0.5);
    float h = sz * (0.5 + 0.3 * cos(a));
    return length(q - vec2(0.0, r * 0.5)) - h;
}
...
void main(){
    ...
    float sdf_result = sdf_heart(p, 0.250000);
    float glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
    float glow_result = apply_glow(sdf_result, glow_pulse);

    vec4 color_result = vec4(vec3(glow_result), 1.0);
    color_result = vec4(color_result.rgb * vec3(1.000000, 0.200000, 0.400000), 1.0);
...
class NeonHeart extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-neon-heart: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-neon-heart', NeonHeart);
})();
