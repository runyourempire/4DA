...
export function SovereignConnectionBlock({ block }: Props) {
  ...
  return (
    <div className="border border-[#D4AF37]/10 rounded-xl bg-[#0A0A0A] p-4 my-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[#A0A0A0] leading-relaxed">{block.insight}</p>
        </div>
      </div>
      ...
      <button
        ...
        className="mt-3 text-[10px] text-[#666] hover:text-[#A0A0A0] transition-colors"
      >
      ...
      {expanded && (
        <div className="mt-2 flex gap-1.5 flex-wrap">
          {block.data_sources.map((src) => (
            <span key={src} className="px-2 py-0.5 bg-[#1F1F1F] text-[10px] text-[#A0A0A0] rounded">
              {src}
            </span>
          ))}
        </div>
