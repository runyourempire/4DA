      ...
      {expanded && (
        <div className="p-4 space-y-2 bg-bg-primary">
          {changelog.added_lines.map((line, i) => (
            <div
              key={`add-${i}`}
              className="px-3 py-2 text-xs text-green-400 bg-green-500/10 border-s-2 border-green-500 rounded-r font-mono"
            >
              + {line}
            </div>
          ))}
          {changelog.removed_lines.map((line, i) => (
            <div
              key={`rem-${i}`}
              className="px-3 py-2 text-xs text-red-400 bg-red-500/10 border-s-2 border-red-500 rounded-r font-mono"
            >
              - {line}
            </div>
          ))}
