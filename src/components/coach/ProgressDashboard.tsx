  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-white">Progress Check-in</h3>
          <p className="text-xs text-[#666666] mt-0.5">Track your STREETS journey and get personalized nudges</p>
        </div>
        <button
        ...
        >
          {loading ? 'Checking in...' : 'Run Progress Check-in'}
        </button>
      </div>

      {/* Dashboard Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <DashboardCard title="Playbook Progress">
          <ProgressBar percentage={playbookPct} label="Overall Completion" />
          {playbookProgress?.modules && playbookProgress.modules.length > 0 ? (
            ...
            <p className="text-[10px] text-[#666666] mt-2 italic">Start the STREETS playbook to track module progress</p>
          )}
        </DashboardCard>

        <DashboardCard title="Profile Completeness">
          <ProgressBar percentage={profilePct} label="Sovereign Profile" />
        ...
        <DashboardCard title="Active Nudges">
          {nudges.length > 0 ? (
            ...
            <p className="text-[10px] text-[#666666] italic">No active nudges. Run a check-in to receive personalized guidance.</p>
          )}
        </DashboardCard>
      </div>
      ...
      {checkInDone && latestCheckIn && (
        <div className="bg-[#141414] border border-[#2A2A2A] rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[10px] text-[#666666] uppercase tracking-wide font-medium">Check-In Results</p>
      ...
      {loading && (
        <div className="flex items-center gap-3 py-4">
          ...
          <span className="text-xs text-[#A0A0A0]">Running progress check-in...</span>
        </div>
      )}
    </div>
  );
