...
function DashboardCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-bg-secondary border border-border rounded-xl p-5">
      <p className="text-[10px] text-text-muted uppercase tracking-wide font-medium mb-3">{title}</p>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="space-y-1.5">
      {content.split('\n').map((line, i) => {
        ...
        if (/^[-*] /.test(t)) return <p key={i} className="text-xs text-text-secondary leading-relaxed pl-3">- {t.slice(2)}</p>;
        return <p key={i} className="text-xs text-text-secondary leading-relaxed">{t}</p>;
      })}
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-text-muted mt-0.5">{t('coach.progress.subtitle')}</p>
        </div>
        <button
            ...
            <div className="mt-3 space-y-2">
              {playbookProgress.modules.slice(0, 4).map((mod) => (
                <div key={mod.module_id} className="flex items-center gap-2">
                  <span className="w-6 h-6 rounded flex items-center justify-center text-[9px] font-bold bg-bg-tertiary text-text-secondary flex-shrink-0">
                    {mod.module_id}
                  </span>
                  <div className="flex-1 h-1 bg-bg-tertiary rounded-full overflow-hidden">
                    <div className="h-full bg-[#22C55E] rounded-full" style={{ width: `${mod.percentage}%` }} />
                  </div>
                  <span className="text-[10px] text-text-muted font-mono w-8 text-right">{Math.round(mod.percentage)}%</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-[10px] text-text-muted mt-2 italic">{t('coach.progress.startPlaybook')}</p>
          )}
        </DashboardCard>
              ...
              return (
                <div key={cat.key} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-text-muted font-mono w-8 text-right">{pct}%</span>
                  </div>
                </div>
              );
            })}
        ...
        <DashboardCard title={t('coach.progress.activeNudges')}>
          {nudges.length > 0 ? (
            <div className="space-y-2 max-h-[200px] overflow-y-auto">
              {nudges.map((nudge) => (
                <div key={nudge.id} className="flex items-start gap-2 bg-bg-primary border border-border rounded-lg px-3 py-2">
                  <span className="text-[#D4AF37] mt-0.5 flex-shrink-0 text-xs">*</span>
                  <p className="text-xs text-text-secondary flex-1 leading-relaxed">{nudge.content}</p>
                  <button
            ...
            <p className="text-[10px] text-text-muted italic">{t('coach.progress.noNudges')}</p>
          )}
        </DashboardCard>
      </div>
      ...
      {checkInDone && latestCheckIn && (
        <div className="bg-bg-secondary border border-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[10px] text-text-muted uppercase tracking-wide font-medium">{t('coach.progress.checkInResults')}</p>
            <span className="text-[10px] text-text-muted">{new Date(latestCheckIn.created_at).toLocaleString()}</span>
          </div>
      ...
      {loading && (
        <div className="flex items-center gap-3 py-4">
          ...
          <span className="text-xs text-text-secondary">{t('coach.progress.runningCheckIn')}</span>
        </div>
      )}
    </div>
  );
