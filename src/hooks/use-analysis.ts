      ...
      const results = await Promise.all([
        ...
        listen<{ briefing: string; model?: string }>('briefing-auto-generated', (event) => {
          ...
          if (data.briefing) {
            ...
            useAppStore.getState().addToast('info', 'New briefing auto-generated from critical signals');
          }
        }),
        ...
        listen<{ type: string; severity: string; description: string }>('anomaly-detected', (event) => {
          ...
          if (severity === 'High' || severity === 'Critical') {
            useAppStore.getState().addToast('warning', `Anomaly: ${description}`);
          }
        }),

        listen<{ item_count: number }>('digest-generated', (event) => {
          useAppStore.getState().addToast('info', `Digest generated: ${event.payload.item_count} items`);
        }),
