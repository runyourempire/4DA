...
export function useAnalysis(
  ...
  useEffect(() => {
    ...
    let unlistenBriefingAutoGenerated: UnlistenFn | null = null;
    let unlistenAnomalyDetected: UnlistenFn | null = null;
    let unlistenDigestGenerated: UnlistenFn | null = null;
    ...
    const setupListeners = async () => {
      ...
      // Auto-generated briefing from monitoring (Fix 1)
      unlistenBriefingAutoGenerated = await listen<{ briefing: string; model?: string }>('briefing-auto-generated', (event) => {
      ...
      // Anomaly detected from monitoring (Fix 5)
      unlistenAnomalyDetected = await listen<{ type: string; severity: string; description: string }>('anomaly-detected', (event) => {
      ...
      // Digest generated from scheduler (Fix 2)
      unlistenDigestGenerated = await listen<{ item_count: number }>('digest-generated', (event) => {
        useAppStore.getState().addToast('info', `Digest generated: ${event.payload.item_count} items`);
      });

      // Progressive streaming: accumulate results during scoring
      unlistenPartialResults = await listen<SourceRelevance[]>('partial-results', (event) => {
        const state = useAppStore.getState();
        if (state.appState.analysisComplete) return; // ignore after final results
    ...
    return () => {
      ...
      if (unlistenBriefingAutoGenerated) unlistenBriefingAutoGenerated();
      if (unlistenAnomalyDetected) unlistenAnomalyDetected();
      if (unlistenDigestGenerated) unlistenDigestGenerated();
      if (unlistenPartialResults) unlistenPartialResults();
    };
