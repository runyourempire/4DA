...
STREETS Community 회원 (월 $29 또는 연 $249)은 개발자들이 다음을 공유하는 프라이빗 커뮤니티에 접근할 수 있습니다:

- **월간 수익 보고:** 실제 수치, 실제 수입원, 실제 과제.
- **수입원 출시:** 무엇을 만들었는지, 어떤 가격을 매겼는지, 무슨 일이 일어났는지.
- **중단 결정:** 무엇을 중단하고 왜 중단했는지. 이것은 가장 가치 있는 게시물 중 일부입니다.
- **성공:** 첫 1달러. 첫 월 $1K. 첫 월 $10K. 이것은 중요합니다.
- **실패:** 실패한 제품. 사라진 클라이언트. 변한 알고리즘. 이것은 더 중요합니다.

100명의 개발자가 동시에 STREETS를 실행하는 곳에서 배우는 것이 어떤 코스, 책, 팟캐스트 — 이 코스를 포함하여 — 보다 빠릅니다.
